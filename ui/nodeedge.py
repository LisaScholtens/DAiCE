from ui import widgets, menus
from PyQt5.Qt import *
from PyQt5.QtCore import *
from PyQt5.QtWidgets import *

class NodeEdgeWidget(QWidget):
    """ """
    ...
    def set_selection(self, obj):
        ...
        if isinstance(obj, str):
            ...
            self.nodeview.selectionModel().setCurrentIndex(index, QItemSelectionModel.SelectCurrent)
    ...
    def construct_widget(self):
        ...
        for i in range(self.edgemodel.columnCount()):
            edgeheader.setSectionResizeMode(i, QHeaderView.ResizeToContents)
        edgeheader.setSectionResizeMode(0, QHeaderView.Stretch)

        self.setLayout(widgets.HLayout([self.nodeview, self.edgeview], [1, 1]))
...
class ListsModel(QAbstractTableModel):
    """
    ...
    def data(self, index, role=Qt.DisplayRole):
        ...
        role : Qt.DisplayRole, optional
            Property of data to display, by default Qt.DisplayRole
...
class NodeListModel(ListsModel):
    ...
    def __init__(self, project, mainwindow):
        ...
        super().__init__(modellist=self.project.bn.nodes, keys=["name","distribution", "parameters_small", "parameters_large", "condition"], labels=["Nodes\n", "Probability\ndistribution", "Distribution\nParameters (s)", "Distribution\nParameters (l)", "Conditional\nValue"])
    ...
    def setData(self, index, value, role=Qt.EditRole):
        ...
        col = index.column()
        row = index.row()
        nodename = getattr(self.modellist[row], "name")
        oldvalue = getattr(self.modellist[row], self.keys[col])

        if oldvalue == value:
            return False

        if self.keys[col] == "name":
            self.signals.name_about_to_change.emit(oldvalue, value)
        elif self.keys[col] == "distribution":
            self.signals.dist_about_to_change.emit(nodename, value)
        elif self.keys[col] == "parameters_small":
            self.signals.dist_param_small_about_to_change.emit(nodename, value)
        elif self.keys[col] == "parameters_large":
            self.signals.dist_param_large_about_to_change.emit(nodename, value)
        elif self.keys[col] == "condition":
            self.signals.cond_val_about_to_change.emit(nodename, value)
        else:
    ...
    def flags(self, index) -> Qt.ItemFlags:
        ...
        if not index.isValid():
            ...
            fl |= Qt.ItemIsEnabled | Qt.ItemIsDragEnabled
...
class EdgeListsModel(ListsModel):
    ...
    def __init__(self, project, mainwindow):
        ...
        super().__init__(
            ...
            keys=["direction_string", "cond_rank_corr", "rank_corr"],
        )
