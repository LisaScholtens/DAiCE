...
class BayesianNetwork(BaseModel):
    ...
    charlist: List[ProjectCharacteristics] = []
    ...
    def add_char(
        ...
        if pax == None:
            pax = str(np.random.rand(1)[0] * 1000)
        ...
        chars = ProjectCharacteristics(project_name=project_name, pax=pax, c_concrete=c_concrete, c_asphalt=c_asphalt, c_ctb=c_ctb, c_sand=c_sand, ils=ils, atc=atc)
    ...
    def add_node(
        ...
        parameters_small: List[float] = None,
        parameters_large: List[float] = None,
        condition: str = None,
    ) -> None:
        """Add node to BN
        ...
        distribution : str, optional
            Probability distribution of the variable. Defaults to None.
        parameters : List[float], optional
            List of parameter values for the probability distribution. Defaults to None.
        """
        if name is None:
        ...
        if distribution is None:
            distribution = "uni"
            parameters_small = [0,1]
            parameters_large = [0, 1]

        if condition is None:
            condition = "None"

        newnode = Node(name=name, edges=edges, distribution=distribution, parameters_small=parameters_small, parameters_large=parameters_large, condition=condition)
    ...
    def reverse_edge(self, parent_name: str, child_name: str) -> bool:
        ...
        -------
        bool
            If reversing was successful (BN still a DAG)
        """
    ...
    def change_distr_type(self, nodename: str,newdist: str) -> None:
        """Change node distribution type"""
        node = self._get_node_by_name(nodename)
        node.distribution = newdist

    def change_distr_param_small(self, nodename: str, newparam: str) -> None:
        node = self._get_node_by_name(nodename)
        node.parameters_small = [float(s) for s in re.findall(r'[-+]?\d*\.?\d+(?:e[-+]?\d+)?', newparam)]

    def change_distr_param_large(self, nodename: str, newparam: str) -> None:
        node = self._get_node_by_name(nodename)
        node.parameters_large = [float(s) for s in re.findall(r'[-+]?\d*\.?\d+(?:e[-+]?\d+)?', newparam)]

    def change_condition(self, name: str, condition: str) -> None:
        try:
            node = self._get_node_by_name(name)
            node.condition = condition
        except:
            for chars in self.charlist:
                if hasattr(chars, name):
                    setattr(chars, name, condition)
    ...
    def to_json(self, path: Path) -> None:
        ...
        with path.open("w") as f:
            f.write(self.model_dump_json(indent=4, exclude={"R", "partcorrs", "edgelist"}))
